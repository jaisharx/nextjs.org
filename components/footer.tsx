...
function FooterLinks({ title, children }) {
  return (
    <VStack alignItems="flex-start">
      ...
      {children.map((item) => (
        <Text color="#8c8c8c" fontSize="14px" key={item}>
          {item}
        </Text>
      ))}
    </VStack>
      ...
      <Container>
        <Grid templateColumns="repeat(4, 1fr)">
          <FooterLinks title="General resources">
            {['Docs', 'Learn', 'Showcase', 'Blog', 'Analytics', 'Next.js Conf']}
          </FooterLinks>
          <FooterLinks title="More">
            {['Commerce', 'Contact Sales', 'Github', 'Releases', 'Telemetry']}
          </FooterLinks>
          <FooterLinks title="About Vercel">
            {['Open Source Software', 'Github', 'Twitter']}
          </FooterLinks>
          <FooterLinks title="Legal">{['Privacy Policy']}</FooterLinks>
        </Grid>
